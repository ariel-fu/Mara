...
class VideoWidget2 extends StatefulWidget {
  ...
  final String title;
  VideoWidget2({Key? key, required this.videoAsset, required this.title}) : super(key:key);
  @override
...
class _VideoWidgetState extends State<VideoWidget2> {
  ...
  void initState() {
    ...
    ourVideo.addListener(() {
      setState(() {
        _isPlaying = ourVideo.value.isPlaying;
      });
    });
  }

  void _videoListener() {
  if (ourVideo.value.isPlaying != _isPlaying) {
    setState(() {
      _isPlaying = ourVideo.value.isPlaying;
    });
  }
  ...
  void dispose() {
    //releasing resources
    ourVideo.removeListener(_videoListener);
    ourVideo.dispose();
    super.dispose();
  }

  @override
  void didUpdateWidget(covariant VideoWidget2 oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.videoAsset != widget.videoAsset) {
      ourVideo.pause(); // Pause the current video
      ourVideo.dispose();
      ourVideo = VideoPlayerController.asset(widget.videoAsset);
      _initializeVideoPlayerFuture = ourVideo.initialize();
      ourVideo.addListener(_videoListener);
      setState(() {
        _isPlaying = false; // Reset playing state when changing video
      });
    }
  }
